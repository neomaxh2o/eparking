...
BACKUP_DIR="$ROOT_DIR/.cleanup-backups"
TS="$(date -u +%Y%m%dT%H%M%SZ)"
BACKUP_PATH="$BACKUP_DIR/staging-pre-clean-$TS"
...
if ! command -v mongodump >/dev/null 2>&1; then
  echo "[error] mongodump no está instalado o no está en PATH"
  exit 1
fi
...
echo "[info] Backup previo: $BACKUP_PATH"
echo "[warn] Esta limpieza es DESTRUCTIVA sobre staging."
echo "[warn] Conserva users y purga playas/abonados/facturación/cajas/tickets/reportes."
read -r -p "Escribe RESET STAGING para continuar: " CONFIRM
...
echo "[step] Generando backup lógico con mongodump..."
mongodump --uri "$MONGODB_URI" --out "$BACKUP_PATH"

echo "[step] Ejecutando limpieza de staging..."
node <<'EOF'
...
echo "[done] Limpieza finalizada."
echo "[done] Backup disponible en: $BACKUP_PATH"
