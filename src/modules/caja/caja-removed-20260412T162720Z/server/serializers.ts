...
export async function serializeTurno(turno: ITurno | (PlainObject & { _id?: unknown })) {
  ...
  return {
    ...
    movimientosAdministrativos: Array.isArray((turno as any).movimientosAdministrativos)
      ? (turno as any).movimientosAdministrativos.map((mov: any) => ({
          invoiceId: String(mov?.invoiceId ?? ''),
          invoiceCode: String(mov?.invoiceCode ?? ''),
          abonadoId: String(mov?.abonadoId ?? ''),
          abonadoNombre: String(mov?.abonadoNombre ?? ''),
          monto: Number(mov?.monto ?? 0),
          paymentMethod: String(mov?.paymentMethod ?? ''),
          paymentReference: String(mov?.paymentReference ?? ''),
          actorUserId: String(mov?.actorUserId ?? ''),
          actorRole: String(mov?.actorRole ?? ''),
          actorName: String(mov?.actorName ?? ''),
          actorEmail: String(mov?.actorEmail ?? ''),
          createdAt: mov?.createdAt
            ? mov.createdAt instanceof Date
              ? mov.createdAt.toISOString()
              : new Date(String(mov.createdAt)).toISOString()
            : undefined,
        }))
      : [],
